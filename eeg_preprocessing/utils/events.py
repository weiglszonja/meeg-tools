...
def get_events_from_raw(raw: mne.io.Raw) -> pd.DataFrame:
    ...
    In case of mismatch it returns a warning message and the extracted
    events unmodified.
    Parameters
    ----------
    ...
    Returns
    -------
    The extracted block and resting events with their respective
    ...
    events = pd.DataFrame(data=events_from_annot,
                          columns=['start_time', 'ignore', 'event_id']).drop(
        columns=['ignore'])
    events['start_time'] = events['start_time'] / raw.info['sfreq']
    events['event'] = events['event_id'].map(event_dict).fillna(
        events['event_id'])

    # Block events
    block_start_indices = events.index[
        events['event_id'].isin([91, 93, 95, 97, 99, 101])]
    block_end_indices = events.index[events['event_id'].isin([52, 152])]
    if len(block_start_indices) != len(block_end_indices):
        print('There is a mismatch between the number of '
    ...
    for start, end in zip(block_start_indices, block_end_indices):
        ...
        if len(block[block['event_id'].isin([19, 119])]) != 7:
            print(
                ...
                'practice blocks for at least one trial!')
            return events

        # take the last practice stimuli time as block start time
        offset_in_seconds = 5
        real_block_start_time = \
            block[block['event_id'].isin([19, 119])][
                'start_time'].values.tolist()[-1]
    ...
    if num_day != '3':
        block_events['sequence'] = events.loc[block_end_indices][
            'event_id'].map(
            {52: 'A', 152: 'B'}).values
    else:
        if (int(subject) % 2) == 0:
            block_events.loc[
                block_events['event_id'].isin([91, 95, 99]), 'sequence'] = 'B'
            block_events.loc[
                block_events['event_id'].isin([93, 97, 101]), 'sequence'] = 'A'
        else:
            block_events.loc[
                block_events['event_id'].isin([91, 95, 99]), 'sequence'] = 'A'
            block_events.loc[
                block_events['event_id'].isin([93, 97, 101]), 'sequence'] = 'B'

    block_events['end_time'] = events.loc[
        block_end_indices, 'start_time'].values

    # Resting events
    resting_events = events[events['event_id'].isin([83, 87])]
    resting_events['end_time'] = events.loc[
        events['event_id'].isin([84, 88]), 'start_time'].values

    # Append resting events to block events
    block_events = block_events.append(resting_events)

    # Add duration of events
    block_events['duration'] = block_events['end_time'] - block_events[
        'start_time']
...
def create_epochs_from_events(raw: mne.io.Raw, events: pd.DataFrame):
    ...
    for idx, event in events.sort_values(
            'start_time').reset_index().iterrows():
    ...
    epochs = mne.Epochs(raw=raw_bandpass,
                        ...
                        picks='eeg',
                        event_id=dict(
                            zip(events['event'], events['event_id'])),
                        baseline=None,
                        tmin=0.,
                        tmax=epoch_duration - (
                                    1 / raw.info['sfreq']),
